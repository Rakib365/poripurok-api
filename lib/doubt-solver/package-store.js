...
export async function deductQuota(userId, n = 1) {
  ...
  try {
    ...
    if (err.name === 'ConditionalCheckFailedException') {
      ...
      const isExpired = cur.validityEnd && cur.validityEnd <= now;
      const isQuotaGone = (cur.quotaRemaining ?? 0) <= 0;
      if (isExpired && isQuotaGone) return { ok: false, reason: 'quota_and_expired' };
      if (isExpired) return { ok: false, reason: 'subscription_expired' };
...
export async function refundQuota(userId, n = 1) {
  try {
    ...
    logger.warn('package-store refundQuota failed', { error: err.message });
  }
}
