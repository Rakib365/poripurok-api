...
async function runLoopCore({ userId, conversationId, messages }) {
  ...
  while (iterations < MAX_ITERATIONS) {
    ...
    if (!validation.ok) {
      ...
      console.warn('════════════════ PARSE-FAIL DUMP ════════════════');
      console.warn(`[AgentLoop] Iter ${iterations}: invalid (${validation.reason}); discard ${consecutiveDiscards}/${MAX_DISCARDS}`);
      console.warn(`[AgentLoop]   hasStructuredObject=${hasObject} textLen=${textLen}`);
      if (hasObject) {
        console.warn(`[AgentLoop]   structuredObject: ${JSON.stringify(llmResponse.object).slice(0, 600)}`);
      }
      console.warn(`[AgentLoop]   rawText[0..600]:\n${snippet}`);
      console.warn('═════════════════════════════════════════════════');
    ...
    console.log(`[AgentLoop] Iter ${iterations}: status=${parsed.status} tools=[${toolNames}] respLen=${(parsed.response || '').length}`);
  ...
  if (iterations >= MAX_ITERATIONS && !finalParsed) {
    console.warn(`[AgentLoop] Hit MAX_ITERATIONS=${MAX_ITERATIONS} without reaching 'done'. Tool trace: ${JSON.stringify(toolTrace.slice(-10))}`);
  }
