    ...
    while (sameKeyRetries < MAX_RETRIES_PER_KEY) {
      try {
        ...
        if (shouldRotate) {
          markKeyFailed(keyIdx);
          totalRetryCount++;
          logger.warn('router gemini key threw, rotating', { keyIndex: keyIdx + 1, keySuffix, statusCode });
          break;
        } else {
          sameKeyRetries++;
          totalRetryCount++;
          logger.warn('router gemini key threw, retrying same key', { keyIndex: keyIdx + 1, keySuffix, statusCode: statusCode || error.message, retry: `${sameKeyRetries}/${MAX_RETRIES_PER_KEY}`, errorDetail: error.message || JSON.stringify(error).slice(0, 500) });
