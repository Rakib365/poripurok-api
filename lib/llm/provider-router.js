...
async function tryGeminiWithCacheOptimizedRetry(request) {
  ...
  for (let keyIdx = 0; keyIdx < keys.length; keyIdx++) {
    ...
    while (sameKeyRetries < MAX_RETRIES_PER_KEY) {
      try {
        logger.info('router trying gemini key', { keyIndex: keyIdx + 1, keySuffix, retry: sameKeyRetries });
        ...
        // 429 (quota exhausted) and 5xx → rotate to next key immediately.
        const shouldRotateKey = statusCode === 429 || (statusCode >= 500 && statusCode < 600);

        if (shouldRotateKey) {
          markKeyFailed(keyIdx);
          totalRetryCount++;
          logger.warn('router gemini key rotating', { keyIndex: keyIdx + 1, keySuffix, statusCode });
          break;
        } else {
          sameKeyRetries++;
          totalRetryCount++;
          logger.warn('router gemini key retrying same key', { keyIndex: keyIdx + 1, keySuffix, statusCode, retry: `${sameKeyRetries}/${MAX_RETRIES_PER_KEY}` });
        ...
        // 429 (quota exhausted) and 5xx → rotate to next key immediately.
        const shouldRotate = typeof statusCode === 'number' && (statusCode === 429 || (statusCode >= 500 && statusCode < 600));

        if (shouldRotate) {
          markKeyFailed(keyIdx);
          totalRetryCount++;
          logger.warn('router gemini key threw, rotating', { keyIndex: keyIdx + 1, keySuffix, statusCode, rawError: error.rawBody ?? error.message });
          break;
        } else {
          sameKeyRetries++;
          totalRetryCount++;
          logger.warn('router gemini key threw, retrying same key', { keyIndex: keyIdx + 1, keySuffix, statusCode: statusCode || error.message, retry: `${sameKeyRetries}/${MAX_RETRIES_PER_KEY}`, rawError: error.rawBody ?? error.message });
